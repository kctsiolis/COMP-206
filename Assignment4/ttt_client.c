...
int main(int argc , char *argv[])
{
	...
	char addr[20] = "127.0.0.1"; //Local address
	unsigned short int port = 8888;

	memset(prev_msg, '\0', 200 );

	if( argc>1 ){
		strcpy( addr, argv[1] ); //Takes address as an argument
	}
	...
	while(1)
	{
		...
		char* ret = fgets (message , 2000 , stdin); //Reading a string from standard input and storing it in message
		if( ret == NULL ){
